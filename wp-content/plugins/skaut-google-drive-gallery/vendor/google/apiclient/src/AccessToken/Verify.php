 ...
 */
namespace Sgdg\Vendor\Google\AccessToken;

use DateTime;
use DomainException;
use Exception;
use Sgdg\Vendor\ExpiredException;
use Sgdg\Vendor\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use Sgdg\Vendor\Firebase\JWT\Key;
use Sgdg\Vendor\Firebase\JWT\SignatureInvalidException;
use Sgdg\Vendor\Google\Auth\Cache\MemoryCacheItemPool;
use Sgdg\Vendor\Google\Exception as GoogleException;
use Sgdg\Vendor\GuzzleHttp\Client;
use Sgdg\Vendor\GuzzleHttp\ClientInterface;
use InvalidArgumentException;
use LogicException;
use Sgdg\Vendor\phpseclib3\Crypt\PublicKeyLoader;
use Sgdg\Vendor\phpseclib3\Crypt\RSA\PublicKey;
// Firebase v2
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('Sgdg\\Vendor\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'Sgdg\\Vendor\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('Sgdg\\Vendor\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('Sgdg\\Vendor\\phpseclib3\\Crypt\\RSA')) {
            return 'Sgdg\\Vendor\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('Sgdg\\Vendor\\phpseclib\\Crypt\\RSA')) {
            return 'Sgdg\\Vendor\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('Sgdg\\Vendor\\phpseclib3\\Math\\BigInteger')) {
            return 'Sgdg\\Vendor\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('Sgdg\\Vendor\\phpseclib\\Math\\BigInteger')) {
            return 'Sgdg\\Vendor\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('Sgdg\\Vendor\\phpseclib3\\Crypt\\AES')) {
            return 'Sgdg\\Vendor\\phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('Sgdg\\Vendor\\phpseclib\\Crypt\\RSA')) {
            return 'Sgdg\\Vendor\\phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('Sgdg\\Vendor\\Crypt_RSA')) {
