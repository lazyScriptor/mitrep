...
use DateTime;
use Exception;
use Sgdg\Vendor\Firebase\JWT\ExpiredException;
use Sgdg\Vendor\Firebase\JWT\JWT;
use Sgdg\Vendor\Firebase\JWT\SignatureInvalidException;
use Sgdg\Vendor\Google\Auth\Cache\MemoryCacheItemPool;
use Sgdg\Vendor\Google\Auth\HttpHandler\HttpClientCache;
use Sgdg\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory;
use Sgdg\Vendor\GuzzleHttp\Psr7\Request;
use Sgdg\Vendor\GuzzleHttp\Psr7\Utils;
use InvalidArgumentException;
use Sgdg\Vendor\phpseclib\Crypt\RSA;
use Sgdg\Vendor\phpseclib\Math\BigInteger;
use Sgdg\Vendor\Psr\Cache\CacheItemPoolInterface;
use RuntimeException;
use Sgdg\Vendor\SimpleJWT\InvalidTokenException;
use Sgdg\Vendor\SimpleJWT\JWT as SimpleJWT;
use Sgdg\Vendor\SimpleJWT\Keys\KeyFactory;
use Sgdg\Vendor\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 3+
        } catch (\Sgdg\Vendor\ExpiredException $e) {
            // firebase/php-jwt 2
        } catch (SignatureInvalidException $e) {
            // firebase/php-jwt 3+
        } catch (\Sgdg\Vendor\SignatureInvalidException $e) {
            // firebase/php-jwt 2
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('Sgdg\\Vendor\\SimpleJWT\\JWT')) {
    ...
    {
        $class = 'Sgdg\\Vendor\\Firebase\\JWT\\JWT';
