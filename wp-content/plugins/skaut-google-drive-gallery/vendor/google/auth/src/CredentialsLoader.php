...
use Sgdg\Vendor\Google\Auth\Credentials\InsecureCredentials;
...
abstract class CredentialsLoader implements FetchAuthTokenInterface, UpdateMetadataInterface
{
    ...
    {
        if (\defined('Sgdg\\Vendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            return ClientInterface::MAJOR_VERSION;
        }
        if (\defined('Sgdg\\Vendor\\GuzzleHttp\\ClientInterface::VERSION')) {
    ...
    public static function makeHttpClient(FetchAuthTokenInterface $fetcher, array $httpClientOptions = [], callable $httpHandler = null, callable $tokenCallback = null)
    {
        if (self::getGuzzleMajorVersion() === 5) {
            $client = new \Sgdg\Vendor\GuzzleHttp\Client($httpClientOptions);
        ...
        $stack = \Sgdg\Vendor\GuzzleHttp\HandlerStack::create();
        $stack->push($middleware);
        return new \Sgdg\Vendor\GuzzleHttp\Client(['handler' => $stack, 'auth' => 'google_auth'] + $httpClientOptions);
    }
    /**
